...
function prepareVisualization(query, type, name, options) {
  return createQuery({ query })
    ...
    .then(({ id: visualizationId, query_id: queryId }) => {
      ...
      cy.getByTestId(`QueryPageVisualization${visualizationId}`).should("exist").find("table").should("exist");
...
describe("Table", () => {
  ...
  describe("Sorting data", () => {
    ...
    it("sorts data by a single column", function () {
      ...
      cy.getByTestId(`QueryPageVisualization${visualizationId}`).find("table th").contains("c").should("exist").click();
    ...
    it("sorts data by a multiple columns", function () {
      ...
      cy.getByTestId(`QueryPageVisualization${visualizationId}`).find("table th").contains("a").should("exist").click();

      cy.get("body").type("{shift}", { release: false });
      cy.getByTestId(`QueryPageVisualization${visualizationId}`).find("table th").contains("b").should("exist").click();
  ...
  it("searches in multiple columns", () => {
    ...
    prepareVisualization(query, "TABLE", "Search", config).then(({ visualizationId }) => {
      cy.getByTestId(`QueryPageVisualization${visualizationId}`).find("table input").should("exist").type("test");
